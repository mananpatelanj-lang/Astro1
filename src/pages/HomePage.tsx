  ...
  return (
    <section className="grid gap-12">
      ...
      <div className="rounded-2xl border-2 border-blue-200 bg-gradient-to-br from-blue-50 to-indigo-50 p-6">
        <h3 className="font-semibold mb-2 text-blue-800">🌟 Gentle reminder</h3>
...
function Badge({ children, color }: { children: React.ReactNode; color: string }) {
  ...
  return (
    <span className={`inline-flex items-center gap-2 rounded-full backdrop-blur px-3 py-1 text-xs border-2 ${colorClasses[color as keyof typeof colorClasses]}`}>
      {children}
    </span>
  );
}
...
function Chip({ children, color }: { children: React.ReactNode; color: string }) {
  ...
  return (
    <span className={`inline-flex items-center rounded-full border-2 px-2.5 py-1 text-xs font-medium ${colorClasses[color as keyof typeof colorClasses]}`}>
      {children}
    </span>
  );
}
...
function Step({ n, title, children, color }: { n: number; title: string; children: React.ReactNode; color: string }) {
  ...
  return (
    <li className={`rounded-xl border-2 p-4 shadow-md hover:shadow-lg transition-shadow ${colorClasses[color as keyof typeof colorClasses]}`}>
      <div className="flex items-center gap-2 mb-1">
        <span className={`h-6 w-6 grid place-items-center rounded-full text-white text-xs font-bold shadow-sm ${numberClasses[color as keyof typeof numberClasses]}`}>{n}</span>
...
function Pillar({ icon, title, children, color }: { icon: string; title: string; children: React.ReactNode; color: string }) {
  const colorClasses = {
    purple: "border-purple-200 bg-gradient-to-br from-purple-50 to-white hover:from-purple-100",
    pink: "border-pink-200 bg-gradient-to-br from-pink-50 to-white hover:from-pink-100",
    orange: "border-orange-200 bg-gradient-to-br from-orange-50 to-white hover:from-orange-100",
    red: "border-red-200 bg-gradient-to-br from-red-50 to-white hover:from-red-100"
  };
  
  return (
    <div className={`rounded-xl border-2 p-5 shadow-md hover:shadow-lg transition-all duration-300 transform hover:-translate-y-1 ${colorClasses[color as keyof typeof colorClasses]}`}>
